...
public class ScreenshotsPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.afterEvaluate {
           ...
           /* Task takeAllScreenshots = project.task("${TASK_PREFIX}",
                    group: GROUP_SCREENSHOTS,
                    description: "Takes screenshots generated by spoon on all the connected devices.")*/
            //List<Task> localesTasks = createSpoonRunTasks(project, screenshotsDirName, properties)
    ...
    private List<Task> createTestsRunTasks(Project project, String screenshotOutputDirName, Properties properties) {
        ...
        locales.each {
            ...
            //TODO: for now we are using only project.screenshots.phone -> later should handle all devices that are set
            Task task = project.tasks.create("${currentLocale}testRunTask", CaptureRunnerTask) {
                ...
                outputPath "$screenshotOutputDirName" // /spoon-output
                instrumentationArgs instrumArgs
                testClassName project.screenshots.screenshotClass
            }

            //TODO: allow specifying the entire path of folder of properties files
            //TODO: commented until we generalise the process of generating jsons based on properties files
